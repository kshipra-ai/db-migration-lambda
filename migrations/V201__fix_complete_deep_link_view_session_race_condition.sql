-- V201: Fix a double-reward race condition in complete_deep_link_view_session.
--
-- V178 (and every version before it) read v_already_completed with a plain SELECT,
-- then later ran an UPDATE ... WHERE is_completed = FALSE to flip the row — but never
-- checked whether that UPDATE actually affected a row before crediting rewards_earned /
-- cash_balance. Under READ COMMITTED, two concurrent or duplicate calls to
-- /qr/complete-view for the same user+campaign could both read is_completed = FALSE
-- before either commits, both pass the guard, and both get credited — the same bug
-- class the V143 migration was written to close, reopened by this structural gap.
--
-- Fix: lock the row with SELECT ... FOR UPDATE up front, so a concurrent call blocks
-- until the first transaction commits and then correctly sees is_completed = TRUE.
-- As defense-in-depth, the completion UPDATE's row count is also checked before any
-- money is credited — if it ever affects zero rows, no reward is awarded.
--
...
BEGIN
    -- SECURITY: lock the row here so a concurrent/duplicate call to this function for
    -- the same user+campaign blocks until this transaction commits, instead of both
    -- reading is_completed = FALSE and both proceeding to award money.
    ...
    IF p_actual_duration_seconds >= v_required_duration THEN
        -- SECURITY: this UPDATE is now the atomic gate — if it affects zero rows
        -- (should not happen given the FOR UPDATE lock above, but checked as
        -- defense-in-depth on a money-critical path), no reward is credited.
        ...
        GET DIAGNOSTICS v_claim_rowcount = ROW_COUNT;

        IF v_claim_rowcount = 0 THEN
            SELECT COALESCE(up.rewards_earned, 0)
            INTO v_total_points
            FROM kshipra_core.user_profile up
            WHERE up.user_id::VARCHAR = p_user_id;
...
'V201: Locks the completion row with FOR UPDATE and gates reward crediting on the completion UPDATE''s row count, closing a double-reward race reopened after V143.';
