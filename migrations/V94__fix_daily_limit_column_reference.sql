-- V94: Fix P1 daily scan limit - max_daily_scans is on qr_campaigns table, not campaigns table
-- The V92 migration incorrectly referenced c.max_daily_scans which doesn't exist
-- This fixes the function to properly join qr_campaigns and use qc.max_daily_scans
...
BEGIN
    ...
    SELECT EXISTS (
        ...
        -- P1 FIX: Join qr_campaigns to get max_daily_scans (it's not on campaigns table!)
        ...
        WHERE 
            ...
            -- P1 FIX: Use qc.max_daily_scans (from qr_campaigns table)
    ...
    IF NOT v_has_unviewed THEN
        ...
        WITH all_campaigns AS (
            ...
            -- P1 FIX: Join qr_campaigns to get max_daily_scans
            LEFT JOIN kshipra_core.qr_campaigns qc ON qc.campaign_id = c.campaign_id
            LEFT JOIN LATERAL (
                SELECT COUNT(*) as daily_scan_count
                FROM kshipra_core.qr_scans qs
...
'P1 FIX (V94): Returns next unviewed APPROVED, NON-DELETED campaign for user that hasnt hit max_daily_scans limit (from qr_campaigns table). Only shows campaigns with review_status = approved AND deleted = false. Filters out campaigns where user has reached max_daily_scans in last 24 hours. If all campaigns viewed or limited, rotates to next available campaign (circular rotation).';
