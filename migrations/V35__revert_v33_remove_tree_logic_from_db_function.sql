-- V34: Revert V33 - Database function should NOT manage tree planting directly
-- Tree planting should be handled by Lambda flow: rewards-lambda → SQS → tree-planting-lambda
-- This migration removes tree calculation logic from complete_deep_link_view_session
-- Instead, we'll need to trigger the rewards Lambda after QR scan completion
...
BEGIN
    ...
    IF v_is_completed AND NOT COALESCE(v_already_completed, false) AND v_user_role != 'brand' THEN
        ...
        -- DO NOT update trees_planted here - that's handled by rewards Lambda
...
'Completes view session, awards points if duration requirement met (first time only) ONLY TO REGULAR USERS (not brands). Tree planting is handled separately by rewards Lambda via SQS. Returns completion status and points awarded.';
