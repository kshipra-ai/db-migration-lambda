-- V130: Fix complete_deep_link_view_session - COALESCE rewards_earned and cash_balance in UPDATE
-- BUG: If user_profile.rewards_earned or cash_balance is NULL (new users),
--      NULL + value = NULL in PostgreSQL, so the UPDATE silently does nothing.
--      The function returns points_awarded > 0 (popup fires) but balance stays 0.
-- FIX: Use COALESCE(up.rewards_earned, 0) and COALESCE(up.cash_balance, 0) in the UPDATE.
...
BEGIN
    ...
    IF p_actual_duration_seconds >= v_required_duration THEN
        ...
        -- Without COALESCE: NULL + value = NULL → balance silently stays 0
...
'V130: Fixed NULL rewards_earned/cash_balance causing silent 0-balance for new users (COALESCE in UPDATE). V129: COALESCE min_view_duration to 10s. V117: Updates qr_scans with actual points.';
