-- V160: Update complete_deep_link_view_session to include location_id in kshipra_earnings
-- Looks up location_id from the corresponding qr_scans record for the same user/campaign/day
-- Based on V148 (preserves rounding fix, double-reward guard, COALESCE fixes)
...
BEGIN
    ...
    -- V160: Look up location_id from the most recent qr_scans record for this user/campaign
    ...
    IF p_actual_duration_seconds >= v_required_duration THEN
        ...
        -- V148 FIX: FLOOR user_rewards and cashback; kshipra gets the remainder
        ...
        -- Record Kshipra's commission with location_id attribution
...
'V160: Add location_id to kshipra_earnings from qr_scans lookup. Preserves V148 rounding, V143 double-reward guard, V129/V130 COALESCE fixes.';
