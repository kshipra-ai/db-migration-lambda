-- V117: Update complete_deep_link_view_session to also update qr_scans table with actual points awarded
-- This ensures scan history shows accurate points for each individual scan
...
BEGIN
    ...
    IF p_actual_duration_seconds >= v_required_duration THEN
        ...
        -- Update view session as completed (use correct column name: completed_at)
        ...
        -- NEW: Update the qr_scans table with actual points awarded
        UPDATE kshipra_core.qr_scans
        SET points_awarded = v_scan_reward_points
        WHERE user_id = p_user_id 
          AND campaign_id = p_campaign_id::VARCHAR
          AND points_awarded = 0  -- Only update records that haven't been updated yet
          AND scanned_at >= CURRENT_DATE;  -- Only today's scans to avoid affecting old data
    ELSE
        -- View not completed - update scan record with 0 points to show it was attempted but not completed
        UPDATE kshipra_core.qr_scans
        SET points_awarded = 0
        WHERE user_id = p_user_id 
          AND campaign_id = p_campaign_id::VARCHAR
          AND scanned_at >= CURRENT_DATE;  -- Only today's scans
    END IF;
...
'Completes view session and updates qr_scans table with actual points awarded. Awards rewards with distribution split if duration requirement met (first time only) ONLY TO REGULAR USERS (not brands). Updates scan records to show actual points earned per scan for accurate scan history.';