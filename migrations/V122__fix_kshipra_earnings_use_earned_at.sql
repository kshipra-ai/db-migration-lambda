-- V122: Fix complete_deep_link_view_session - Use earned_at instead of created_at for kshipra_earnings
-- The kshipra_earnings table uses earned_at column, not created_at
...
BEGIN
    ...
    IF p_actual_duration_seconds >= v_required_duration THEN
        ...
        -- Record Kshipra's commission (FIXED: use earned_at instead of created_at)
...
'Completes view session and updates qr_scans table with actual points awarded. Awards rewards with distribution split if duration requirement met. Fixed: use earned_at (default timestamp) instead of created_at for kshipra_earnings INSERT.';
