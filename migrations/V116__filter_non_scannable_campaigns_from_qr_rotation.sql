-- V116: Filter non-scannable campaigns from QR rotation
-- Excludes store free ads (scannable=false) from QR scan results
-- Free store ads appear in carousel but cannot be scanned for rewards
...
BEGIN
    -- Check if user has any unviewed SCANNABLE campaigns that haven't hit daily limits
    SELECT EXISTS (
        SELECT 1
        ...
        WHERE 
            ...
            AND c.scannable = true  -- NEW: Only scannable campaigns (excludes free store ads)
    ...
    -- If user has viewed all scannable campaigns OR hit daily limits, rotate to next available one
    IF NOT v_has_unviewed THEN
        ...
        -- Return the next SCANNABLE campaign after the last viewed one (circular rotation)
        RETURN QUERY
        WITH all_campaigns AS (
            ...
            WHERE 
                ...
                AND c.deleted = false
                AND c.review_status = 'approved'
                AND c.scannable = true  -- NEW: Only scannable campaigns
                AND c.landing_url IS NOT NULL
                AND (c.start_date IS NULL OR c.start_date <= CURRENT_TIMESTAMP)
                AND (c.end_date IS NULL OR c.end_date >= CURRENT_TIMESTAMP)
                AND (qc.max_daily_scans IS NULL OR qc.max_daily_scans = 0 OR daily_scans.daily_scan_count < qc.max_daily_scans)
        ...
        -- If no campaigns after last viewed, wrap around to first campaign
        IF NOT FOUND THEN
            RETURN QUERY
            SELECT 
                c.campaign_id::UUID,
    ...
    -- Return next unviewed SCANNABLE campaign (original logic)
    RETURN QUERY
    SELECT 
        c.campaign_id::UUID,
        c.landing_url,
...
'V116: Returns next unviewed SCANNABLE campaign for QR scans. Filters out non-scannable campaigns (e.g., free store ads). Only shows campaigns with scannable=true, review_status=approved, and deleted=false. Respects daily scan limits and circular rotation.';
