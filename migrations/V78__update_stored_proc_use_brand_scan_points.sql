-- V78: Update complete_deep_link_view_session to use brand's configured scan_reward_points
-- and return split amounts for mobile app display
...
BEGIN
    ...
    IF p_actual_duration_seconds >= v_required_duration THEN
        ...
        SET 
            view_completed_at = CURRENT_TIMESTAMP,
            is_completed = TRUE,
        ...
        SET 
            rewards_earned = rewards_earned + v_user_rewards_cents,
            cash_balance = cash_balance + (v_cashback_cents::DECIMAL / 100),
            updated_at = CURRENT_TIMESTAMP
        WHERE user_id::VARCHAR = p_user_id;
    ...
    SELECT COALESCE(rewards_earned, 0)
    INTO v_total_points
