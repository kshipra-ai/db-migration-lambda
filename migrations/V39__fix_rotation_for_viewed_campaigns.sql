-- V39: Fix campaign rotation to cycle through all viewed campaigns
-- When all campaigns are viewed, return the next one after the most recently viewed
-- This enables proper rotation instead of always showing the first campaign
...
BEGIN
    ...
    IF NOT v_has_unviewed THEN
        ...
        last_campaign_position AS (
            SELECT row_num
            FROM all_campaigns
            WHERE campaign_id = v_last_viewed_campaign_id
        )
        SELECT 
            ac.campaign_id,
            ac.landing_url,
