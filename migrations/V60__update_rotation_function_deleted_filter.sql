-- V60: Update get_next_unviewed_campaign to filter out deleted campaigns
-- Ensures deleted campaigns are never shown to users
...
'Returns next unviewed APPROVED, NON-DELETED campaign for user. Only shows campaigns with review_status = approved AND deleted = false. If all campaigns viewed, rotates to next campaign after most recently viewed (circular rotation).';
