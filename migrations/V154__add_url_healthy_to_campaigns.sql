-- V154: Add url_healthy flag to campaigns table and update stored proc.
-- When a QR scan detects a campaign's landing URL is unreachable, it marks
-- url_healthy = false. The stored proc filters out unhealthy URLs so
-- subsequent users skip them immediately. Admins/partners are notified
-- via email to fix the URL, then can re-enable the campaign.

ALTER TABLE kshipra_core.campaigns ADD COLUMN IF NOT EXISTS url_healthy BOOLEAN DEFAULT TRUE;

COMMENT ON COLUMN kshipra_core.campaigns.url_healthy IS 'Whether the landing URL is reachable. Set to false when a scan detects the URL is down. Filtered in campaign rotation.';

-- Recreate stored proc with url_healthy filter
...
BEGIN
    ...
    SELECT EXISTS (
        ...
        WHERE 
            ...
            AND (c.view_count IS NULL OR c.total_engagements < c.view_count)
...
'V154: Added url_healthy filter. Campaigns with unreachable landing URLs (url_healthy=false) are excluded from rotation. First scan to detect a dead URL marks it unhealthy and notifies partner+admin.';
