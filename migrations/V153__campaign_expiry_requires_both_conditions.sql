-- V153: Campaign expiry requires BOTH conditions (end_date passed AND view_count reached)
-- Previously the rotation stored proc independently filtered by end_date, hiding
-- date-expired campaigns even when their view_count hadn't been reached yet.
-- The auto-disable sweep (POST /campaigns/auto-disable-expired) already enforces
-- the AND condition correctly, so we just need to stop the rotation from hiding
-- campaigns that are still is_active=true.
...
BEGIN
    ...
    -- NOTE: end_date is NOT checked here. Campaigns stay active until the auto-disable
    -- sweep deactivates them (requires BOTH end_date passed AND view_count reached).
    SELECT EXISTS (
        SELECT 1
...
'V153: Campaign expiry business rule - campaigns with view_count reached are excluded from rotation even if still is_active. end_date alone does NOT exclude a campaign. The auto-disable sweep sets is_active=false only when BOTH end_date passed AND view_count reached.';
